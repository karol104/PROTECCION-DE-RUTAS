...
const submitFormCompanies = async () => {
  ...
  // Si es válido, llamar a la función original de submit
  await submitForm()
}
...
watch(() => formData.value?.department, (newDep) => {
  ...
  if (depName) {
    ...
    if (formData.value) formData.value.city = ''
  } else {
    citiesOptions.value = []
    if (formData.value) formData.value.city = ''
  }
...
async function populateFormForEditCompanies(row) {
  ...
  // Intentar extraer ciudad desde selectedEntity.location si existe
  const existingLocation = selectedCompanies?.location || ''
  if (existingLocation) {
